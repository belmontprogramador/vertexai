...
// Busca o contato e retorna o primeiro lead encontrado com tag "boletopipeline"
// Busca o contato e retorna o primeiro lead encontrado com a tag "boletopipeline"
...
async function findContactAndLeadByPhone(phone) {
  ...
  for (const l of leads) {
    ...
    if (hasBoletoTag && isAtivo && !hasBlockedTag) {
      return {
        contact,
        lead: {
          id: leadData.id,
...
async function pipelineRemarketingBoleto(phone) {
  const { contact, lead } = await findContactAndLeadByPhone(phone);

  if (!lead) {
    throw new Error("❌ Nenhum lead com tag 'boletopipeline' encontrado para este contato.");
  }
...
module.exports = { pipelineRemarketingBoleto };
