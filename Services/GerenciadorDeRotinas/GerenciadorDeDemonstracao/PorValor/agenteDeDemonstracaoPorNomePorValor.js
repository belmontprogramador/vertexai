...
const agenteDeDemonstracaoPorNomePorValor = async ({ sender, msgContent, modeloMencionado }) => {    
  ...
  await delay(3000);
    await sendBotMessage(sender, `Opa ${nome}, infelizmente não trabalhos com esse modelo no momento `);
    await delay(2000);
    await sendBotMessage(sender, `📦 No momento, não temos o modelo que você mencionou. Trabalhamos com Linha *Realme, Redmi e Poco*. Pode escolher outro ou digitar um valor que você pretende investir? `);
    await delay(1000);
...
module.exports = { agenteDeDemonstracaoPorNomePorValor
