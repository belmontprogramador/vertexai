...
// ▶️ Como CLI:  node scripts/moverContatosParadosParaRemarketing.cli.js
// ▶️ Como módulo: const { moverContatosParados } = require('./scripts/moverContatosParadosParaRemarketing.cli');
...
try {
  ...
  try {
    ...
    if (require.main === module) {
      ...
      throw new Error("Ajuste os paths dos imports de redisService/normalizePhone neste script: " + e2.message);
    }
  }
}
...
// (token e headers serão lidos dinamicamente via buildHeaders())
...
// headers dinâmicos (evita process.exit ao ser importado)
function buildHeaders() {
  const token = process.env.KOMMO_TOKEN;
  if (!token) {
    throw new Error("KOMMO_TOKEN ausente. Carregue seu .env antes de chamar moverContatosParados().");
  }
  return {
...
async function salvarRelatorios({ movedRows, allRows }) {
  ...
  if (allRows.length) {
    paths.todosCsv  = await salvarCSV("todos-remarketing", headerAll, allRows);
    paths.todosJson = await salvarJSON("todos-remarketing", allRows);
  } else {
    console.log("ℹ️ Nenhum contato processado para o relatório completo.");
  }

  if (movedRows.length) {
    paths.movCsv  = await salvarCSV("movidos-remarketing", headerMoved, movedRows);
    paths.movJson = await salvarJSON("movidos-remarketing", movedRows);
  } else {
...
async function moverContatosParados() {
  // valida token aqui (para não matar quem apenas importa o módulo)
  if (!process.env.KOMMO_TOKEN) {
    throw new Error("KOMMO_TOKEN ausente. Defina no .env antes de executar moverContatosParados().");
  }
  ...
  const paths = await salvarRelatorios({ movedRows, allRows });
  console.log(`🏁 Fim do job | movidos=${movidos} | ignorados=${ignorados} | erros=${erros} | pause_global=${pausedGlobal}`);

  // retorno útil quando usado como módulo
  return { movidos, ignorados, erros, pausedGlobal, paths, totalProcessados: allRows.length };
}
...
//
// =========================
// CLI (somente se rodar direto)
// =========================
if (require.main === module) {
  moverContatosParados()
    .then((resumo) => {
      console.log("✅ Execução concluída (CLI). Resumo:", resumo);
      process.exit(0);
    })
    .catch((err) => {
