...
async function moverLeadParaClientesVertexSuporte(telefoneSemMais) {
  try {
    ...
    if (!contato) {
      ...
      return await criarLeadSuporte(novoContato.id, nome);
    }
    ...
    if (!leadExistente) {
      return await criarLeadSuporte(contato.id, nome);
    }

    const precisaAtualizar =
      leadExistente.status_id !== STATUS_SUPORTE ||
      ...
      const payload = [
        {
          ...
          status_id: STATUS_SUPORTE,
          ...(nome && leadExistente.name !== nome ? { name: nome } : {}),
        },
      ];
      await axios.patch(`${KOMMO_BASE_URL}/api/v4/leads`, payload, { headers });
      console.log(`🔁 Lead ${leadExistente.id} atualizado para 'Suporte'`);
    } else {
      console.log("✅ Lead já está em 'Suporte'. Nenhuma ação necessária.");
    }
  } catch (err) {
...
async function criarLeadSuporte(contactId, nome) {
  const payload = [
    {
      name: nome,
      status_id: STATUS_SUPORTE,
...
module.exports = { moverLeadParaClientesVertexSuporte };

// Execução direta via terminal
if (require.main === module) {
  const telefone = process.argv[2];
  if (!telefone) {
    console.error("⚠️ Informe o número de telefone como argumento. Ex: node moverLeadParaClientesVertexSuporte.js 5521983735922");
    process.exit(1);
  }
  moverLeadParaClientesVertexSuporte(telefone);
}
