...
const STATUS_PIX_CARTAO = 74988076;
...
async function moverLeadParaClientesVertexPix(telefoneSemMais) {
  try {
    ...
    if (!contato) {
      ...
      return await criarLeadPixCartao(novoContato.id, nome);
    }
    ...
    if (!leadExistente) {
      return await criarLeadPixCartao(contato.id, nome);
    }

    const precisaAtualizar =
      leadExistente.status_id !== STATUS_PIX_CARTAO ||
      ...
      const payload = [
        {
          ...
          status_id: STATUS_PIX_CARTAO,
          ...(nome && leadExistente.name !== nome ? { name: nome } : {}),
        },
      ];
      await axios.patch(`${KOMMO_BASE_URL}/api/v4/leads`, payload, { headers });
      console.log(`🔁 Lead ${leadExistente.id} atualizado para 'Celular Pix/Cartão'`);
    } else {
      console.log("✅ Lead já está em 'Celular Pix/Cartão'. Nenhuma ação necessária.");
    }
  } catch (err) {
...
async function criarLeadPixCartao(contactId, nome) {
  const payload = [
    {
      name: nome,
      status_id: STATUS_PIX_CARTAO,
...
module.exports = { moverLeadParaClientesVertexPix };

// Execução direta via terminal
if (require.main === module) {
  const telefone = process.argv[2];
  if (!telefone) {
    console.error("⚠️ Informe o número de telefone como argumento. Ex: node moverLeadParaClientesVertexPix.js 5521983735922");
    process.exit(1);
  }
  moverLeadParaClientesVertexPix(telefone);
}
