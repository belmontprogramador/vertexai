...
const STATUS_CELULAR_BOLETO = 74988072;
...
async function moverLeadParaClientesVertexBoleto(telefoneSemMais) {
  try {
    ...
    if (!contato) {
      ...
      return await criarLeadCelularBoleto(novoContato.id, nome);
    }
    ...
    if (!leadExistente) {
      return await criarLeadCelularBoleto(contato.id, nome);
    }

    const precisaAtualizar =
      leadExistente.status_id !== STATUS_CELULAR_BOLETO ||
      ...
      const payload = [
        {
          ...
          status_id: STATUS_CELULAR_BOLETO,
          ...(nome && leadExistente.name !== nome ? { name: nome } : {}),
        },
      ];
      await axios.patch(`${KOMMO_BASE_URL}/api/v4/leads`, payload, { headers });
      console.log(`🔁 Lead ${leadExistente.id} atualizado para 'Celular Boleto'`);
    } else {
      console.log("✅ Lead já está em 'Celular Boleto'. Nenhuma ação necessária.");
    }
  } catch (err) {
...
async function criarLeadCelularBoleto(contactId, nome) {
  const payload = [
    {
      name: nome,
      status_id: STATUS_CELULAR_BOLETO,
...
module.exports = { moverLeadParaClientesVertexBoleto };

if (require.main === module) {
  const telefone = process.argv[2];
  if (!telefone) {
    console.error("⚠️ Informe o número de telefone como argumento. Ex: node moverLeadParaClientesVertexBoleto.js 5521983735922");
    process.exit(1);
  }
  moverLeadParaClientesVertexBoleto(telefone);
}
