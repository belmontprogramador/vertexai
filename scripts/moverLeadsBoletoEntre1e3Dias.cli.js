#!/usr/bin/env node
// moverLeadsBoletoEntre1e3Dias.cli.js
...
try {
  ...
  ({ getUltimaInteracaoPorLeadId } = require("../Services/redisService"));
} catch {
  try {
    ({
      getTodosUsuariosComStageESemInteracao,
    ...
    ({ getUltimaInteracaoPorLeadId } = require("../../redisService"));
  } catch (e2) {
    console.error("❌ Ajuste os paths dos imports (redisService/normalizePhone/getUltimaInteracaoPorLeadId).", e2.message);
    process.exit(1);
  }
}
...
function labelStatus(id) { return STATUS_LABELS[id] || String(id); }
...
// ------- decisão (por telefone) -------
//
// Retorna:
//   mover=false | motivo=... | detalhe=...   (quando não deve mover)
//   mover=true  | candidateLeadId=...        (quando deve mover)
//
// Regras BOLETO:
// - Considera apenas leads nos pipelines 7471539 ou 11573376
// - Status não pode estar na lista de bloqueios do pipeline atual
// - Lead precisa ter a tag 'boletopipeline'
// - A última interação do LEAD (Redis) deve estar entre 1 e 3 dias
// - Se múltiplos elegíveis, escolhe o último elegível iterado
//
async function deveMoverContato(telefone) {
  ...
  for (const lb of leadsBasicos) {
    ...
    if (!CONSIDER_PIPELINES.has(pipelineId)) continue; // fora dos pipelines alvo

    if (statusBloqueado(pipelineId, statusId)) {
      // Apenas loga e segue procurando outro lead elegível desse contato
      console.log(`🔎 Lead ${lb.id} bloqueado: pipeline=${pipelineId} status=${statusId} (${labelStatus(statusId)})`);
      continue;
    }

    // exige tag 'boletopipeline'
    const tags = det._embedded?.tags || [];
    const temTag = tags.some(t => (t.name || "").toLowerCase() === TAG_BOLETO.toLowerCase());
    if (!temTag) {
      console.log(`🔎 Lead ${lb.id} sem tag '${TAG_BOLETO}', ignorando.`);
      continue;
    }

    // interação 1~3 dias por lead
    ...
    if (!interacaoOK) {
      console.log(`🔎 Lead ${lb.id} fora da janela 1–3 dias, ignorando.`);
      continue;
    }

    // Elegível — mantém o último elegível encontrado
...
async function main() {
  ...
  for (const { sender } of contatosParados) {
    ...
    try {
      ...
      await rateGate("PATCH"); // gap antes do PATCH final
